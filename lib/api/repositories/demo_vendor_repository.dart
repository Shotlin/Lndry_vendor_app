...
class DemoVendorRepository implements VendorRepository {
  ...
  Map<String, dynamic>? _pendingCapacityRequest;
  ...
  Future<VerifyOtpVendorResult> verifyOtp({
    ...
    return VerifyOtpVendorResult(
      ...
      vendor: _profile,
      userPhone: phone,
      // Demo mode always shows the full dashboard, not the restricted
      // rider view.
  ...
  Future<VendorModel> updateProfile({
    ...
    _profile = _profile.copyWith(
      ...
      logoUrl: logoUrl ?? _profile.logoUrl,
      coverImageUrl: bannerUrl ?? _profile.coverImageUrl,
  ...
  Future<String> uploadImage(XFile file, {String? folder}) async {
    // No real backend in demo mode — just echo the local file path back so
    // the UI can preview it immediately via Image.file.
    return file.path;
  }
  ...
  // ── Onboarding Application (demo stub — not part of the live demo path) ──────
  VendorApplicationModel _demoApplication = const VendorApplicationModel(
    id: 'demo-application',
    ownerId: 'demo-owner',
    name: 'Demo Laundry',
    status: 'DRAFT',
  ...
  @override
  Future<VendorApplicationModel> getOrCreateApplication() async => _demoApplication;

  @override
  Future<VendorApplicationModel> getApplicationMe() async => _demoApplication;
  ...
  Future<VendorApplicationModel> updateApplicationOwner(
    ...
    _demoApplication = _demoApplication.copyWith(
      ownerName: ownerName ?? _demoApplication.ownerName,
      email: email ?? _demoApplication.email,
      phone: phone ?? _demoApplication.phone,
      bankAccountNumber: bankAccountNumber ?? _demoApplication.bankAccountNumber,
      bankIfsc: bankIfsc ?? _demoApplication.bankIfsc,
      bankName: bankName ?? _demoApplication.bankName,
      bankHolderName: bankHolderName ?? _demoApplication.bankHolderName,
    );
    return _demoApplication;
  }
  ...
  Future<VendorApplicationModel> updateApplicationBusiness(
    ...
    _demoApplication = _demoApplication.copyWith(
      name: name ?? _demoApplication.name,
      description: description ?? _demoApplication.description,
      gstNumber: gstNumber ?? _demoApplication.gstNumber,
      panNumber: panNumber ?? _demoApplication.panNumber,
    );
    return _demoApplication;
  }
  ...
  Future<VendorApplicationModel> updateApplicationLocation(
    ...
    _demoApplication = _demoApplication.copyWith(
      addressLine1: addressLine1 ?? _demoApplication.addressLine1,
      addressLine2: addressLine2 ?? _demoApplication.addressLine2,
      city: city ?? _demoApplication.city,
      state: state ?? _demoApplication.state,
      pincode: pincode ?? _demoApplication.pincode,
      lat: lat ?? _demoApplication.lat,
      lng: lng ?? _demoApplication.lng,
    );
    return _demoApplication;
  }
  ...
  Future<VendorApplicationModel> updateApplicationRadius(
    ...
    _demoApplication = _demoApplication.copyWith(
      requestedServiceRadiusKm: requestedRadiusKm,
      requestedDailyCapacity: requestedDailyCapacity ?? _demoApplication.requestedDailyCapacity,
    );
    return _demoApplication;
  }
  ...
  Future<String> uploadApplicationDocument(
    ...
    _demoApplication = _demoApplication.copyWith(
      documents: [
        ..._demoApplication.documents,
        VendorApplicationDocumentModel(
          id: 'demo-doc-${_demoApplication.documents.length}',
          documentType: documentType,
          status: 'APPROVED',
        ),
      ],
    );
    return 'demo-doc-${_demoApplication.documents.length - 1}';
  }
  ...
  Future<void> deleteApplicationDocument(String appId, String documentId) async {
    _demoApplication = _demoApplication.copyWith(
      documents:
          _demoApplication.documents.where((d) => d.id != documentId).toList(),
    );
  }

  @override
  Future<VendorApplicationModel> submitApplication(String appId) async {
    _demoApplication = _demoApplication.copyWith(status: 'WAITING_FOR_APPROVAL');
    return _demoApplication;
  }
  ...
  Future<List<CategoryModel>> getServiceCategories() async {
    return const [
      CategoryModel(id: 'demo-cat-wash', name: 'Wash & Fold', description: '', icon: ''),
      CategoryModel(id: 'demo-cat-iron', name: 'Wash & Iron', description: '', icon: ''),
      CategoryModel(id: 'demo-cat-dry', name: 'Dry Cleaning', description: '', icon: ''),
    ];
  }
  ...
  Future<List<GarmentTypeModel>> getGarmentTypes(String categoryId) async {
    const demoSubcategories = {
      'demo-cat-wash': [
        GarmentTypeModel(id: 'demo-gt-wash-fold', name: 'Wash & Fold', categoryId: 'demo-cat-wash', unit: 'kg', demoPrice: 80),
      ],
      'demo-cat-iron': [
        GarmentTypeModel(id: 'demo-gt-wash-iron', name: 'Wash & Steam Iron', categoryId: 'demo-cat-iron', unit: 'kg', demoPrice: 120),
      ],
      'demo-cat-dry': [
        GarmentTypeModel(id: 'demo-gt-shirt', name: 'Shirt', categoryId: 'demo-cat-dry', unit: 'piece', demoPrice: 90),
      ],
    };
    return demoSubcategories[categoryId] ?? const [];
  }
  ...
  Future<void> bulkUpsertGarmentRates(String serviceId, List<GarmentRateUpsertItem> items) async {
    ...
    for (final item in items) {
      rates.removeWhere((r) => r['garment_rate_id'] == item.garmentTypeId);
      rates.add({
        'garment_rate_id': item.garmentTypeId,
        'garment_name': item.garmentTypeId,
  ...
  Future<Map<String, dynamic>> reconcileOrder(
    ...
    _orders[idx] = _orders[idx].copyWith(
      status: OrderStatus.reconciliationPending,
      cancellationReason: adjustmentReason,
    );
    return {
      'orderId': orderId,
      'status': 'RECONCILIATION_PENDING',
      'reconciliation_id': 'demo-reconciliation-$orderId',
      'previous_payable_amount_paise': (_orders[idx].total * 100).round(),
      'proposed_payable_amount_paise': (_orders[idx].total * 100).round(),
    };
  }
  ...
  Future<List<EmployeeModel>> getRiders() async {
    return _employees.where((e) => e.role == 'VENDOR_RIDER').toList();
  }
  ...
  Future<RiderJobModel> getRiderJobDetail(String orderId) async {
    throw const ApiException(message: 'No demo job data available');
  }
  ...
  Future<Map<String, dynamic>> collectBalance(String orderId) async {
    return {'orderId': orderId, 'balance_collected_paise': 0};
  }
  ...
  Future<Map<String, dynamic>> getCapacity() async {
    return {
      'max_orders_per_day': _maxOrdersPerDay,
      'pending_capacity_request': _pendingCapacityRequest,
    };
  }

  @override
  Future<void> requestCapacityChange(int maxOrdersPerDay) async {
    // Mirrors the real backend: doesn't apply instantly, just records a
    // pending request (no demo-mode admin exists to approve it).
    _pendingCapacityRequest = {
      'id': 'demo-capacity-request',
      'requested_daily_limit': maxOrdersPerDay,
      'status': 'PENDING',
  ...
  Future<Map<String, dynamic>> getSupportTicket(String id) async {
    return _demoTicketsList.firstWhere((t) => t['id'] == id, orElse: () => _demoTicketsList.first);
  }

  @override
  Future<Map<String, dynamic>> rateSupportTicket(String id, int rating) async {
    final idx = _demoTicketsList.indexWhere((t) => t['id'] == id);
    if (idx != -1) {
      _demoTicketsList[idx] = {
        ..._demoTicketsList[idx],
        'rating': rating,
        'rated_at': DateTime.now().toIso8601String(),
        'status': 'CLOSED',
      };
      return _demoTicketsList[idx];
    }
    return {};
  }
  ...
  Future<Map<String, dynamic>> followUpSupportTicket(String id, String message) async {
    ...
    if (idx != -1) {
      _demoTicketsList[idx] = {
        ...
        'vendor_replied_at': DateTime.now().toIso8601String(),
        'status': 'OPEN',
      };
      return _demoTicketsList[idx];
    }
