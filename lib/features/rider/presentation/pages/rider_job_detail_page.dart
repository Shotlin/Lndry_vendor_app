...
class _RiderJobDetailPageState extends ConsumerState<RiderJobDetailPage> {
  ...
  Future<void> _navigate(BuildContext context, RiderJobModel job) async {
    ...
    if (!job.hasLocation) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n.riderNoLocationSnack)),
      );
      return;
    }
    final uri = Uri.parse(
    ...
    if (!launched && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n.riderCouldNotOpenMaps)),
      );
    }
  }
  ...
  Future<void> _startLeg(RiderJobModel job) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(job.isPickup ? l10n.riderCouldNotStartPickup('$e') : l10n.riderCouldNotStartDelivery('$e'))),
        );
      }
    } finally {
  ...
  void _cancelPickup(BuildContext context) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(AppLocalizations.of(context).riderCancelPickupNotAvailable)),
    );
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () => context.canPop() ? context.pop() : context.go(AppRoutes.riderJobs),
        ),
        title: Text(
      ...
      body: jobAsync.when(
        ...
        error: (err, _) => Center(child: Text(l10n.riderFailedToLoadJob('$err'))),
      ),
    );
  }
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: (job.isPickup ? AppColors.secondary : AppColors.primary)
                            .withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(20.r),
                      ),
                      child: Text(
                        job.isPickup ? l10n.riderPickupBadge : l10n.riderDeliveryBadge,
                ...
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Icon(Icons.location_on_outlined, size: 18.r, color: AppColors.textSecondary),
                    SizedBox(width: 6.w),
                    Expanded(
                      child: Text(job.addressLine, style: AppTypography.bodyMedium),
                    ),
                  ],
                ),
                if (job.scheduledLabel != null && job.scheduledLabel!.isNotEmpty) ...[
                  ...
                  Row(
                    children: [
                      Icon(Icons.schedule_rounded, size: 18.r, color: AppColors.primary),
                      SizedBox(width: 6.w),
                      Text(
                        job.scheduledLabel!,
          ...
          if (!job.isPickup) ...[
            ...
            _PaymentStatusBanner(
              job: job,
              isDark: isDark,
              onRefresh: () => ref.invalidate(riderJobDetailProvider(job.orderId)),
            ),
          ],
          SizedBox(height: 16.h),
          if (job.lines.isNotEmpty) ...[
            Text(l10n.riderItemsHeader, style: AppTypography.bodyLarge.copyWith(fontWeight: FontWeight.bold)),
            SizedBox(height: 8.h),
            Container(
              ...
              child: Column(
                children: job.lines
                    .map((l) => Padding(
                          ...
                          child: Row(
                            children: [
                              Expanded(child: Text('${l.quantity} × ${l.garmentName}')),
                              Text(l.unit, style: TextStyle(color: AppColors.textSecondary)),
                            ],
                          ),
                        ))
                    .toList(),
            ...
            child: ElevatedButton(
              onPressed: awaitingStart
                  ...
                  : () {
                      if (job.isPickup) {
                        context.push(
                          AppRoutes.riderMeasurements.replaceFirst(':orderId', job.orderId),
                          extra: job,
                        );
                      } else if (job.isCod && job.hasBalanceDue) {
                        context.push(
                          AppRoutes.riderCollectBalance.replaceFirst(':orderId', job.orderId),
                          extra: job,
                        );
                      } else if (job.hasBalanceDue) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(l10n.riderCustomerPaymentPendingSnack),
                          ),
                        );
                      } else {
                        context.push(
                          AppRoutes.riderDeliveryPhoto.replaceFirst(':orderId', job.orderId),
                          extra: job,
                        );
                      }
                    },
              ...
              child: _isStartingLeg
                  ...
                  : Text(
                      awaitingStart
                          ? (job.isPickup ? l10n.riderStartPickupButton : l10n.riderStartDeliveryButton)
                          : (job.isPickup ? l10n.riderPickUpButton : l10n.riderMarkDeliveredButton),
                    ),
            ),
          ),
          if (job.isPickup) ...[
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            isPending ? Icons.error_outline_rounded : Icons.check_circle_outline_rounded,
            color: color,
            size: 20.r,
          ),
          SizedBox(width: 10.w),
          Expanded(
            child: Text(
              isPending
                  ? l10n.riderPaymentPending(CurrencyUtils.formatDecimal(balanceRupees))
