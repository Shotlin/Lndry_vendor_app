...
void main() {
  group('Backend Response Parsing Tests (String vs Num tolerance)', () {
    ...
    test('10. parseApplicationForTest with string lat/lng/radius and nested documents', () {
      final applicationJson = {
        ...
        "lat": "22.580000",
        "lng": "88.370000",
        "requested_service_radius_km": "10.00",
        "approved_service_radius_km": "8.00",
        "documents": [
          {
            "id": "doc-1",
            "document_type": "owner_identity",
            "status": "PENDING",
          }
        ]
      };
      final application = repo.parseApplicationForTest(
        applicationJson,
        missingSteps: const ['document:shop_photo'],
      );
      expect(application.id, "app-1");
      expect(application.status, "WAITING_FOR_APPROVAL");
      expect(application.lat, 22.58);
      expect(application.lng, 88.37);
      expect(application.requestedServiceRadiusKm, 10.0);
      expect(application.approvedServiceRadiusKm, 8.0);
      expect(application.documents.length, 1);
      expect(application.documents.first.documentType, "owner_identity");
      expect(application.missingSteps, ['document:shop_photo']);
    });
  });
}
